...
pub async fn run_big_data_example() -> anyhow::Result<()> {
    ...
    println!("\n========== 获取主动买入/卖出情况 ==========");
    let taker_volume = big_data
        .get_taker_volume("BTC", "SPOT", None, None, Some("5m"))
        .await?;
    println!("BTC SPOT 主动买入/卖出量 (5分钟粒度): {:?}", taker_volume);

    println!("\n========== 获取合约主动买入/卖出情况 ==========");
    let contract_volume = big_data
        .get_taker_volume_contract("BTC-USDT-SWAP", Some("5m"), None, None, None, Some("5"))
        .await?;
    println!(
        "BTC-USDT-SWAP 合约主动买入/卖出量 (5分钟粒度, 最近5条): {:?}",
        contract_volume
    );
